...
data_prioritized <- geo_enriched %>%
  ...
  mutate(
    ...
    prio_i1 = sum(c(b_pop_d, b_ci, b_d_sites, b_n_ind, b_n_risk), na.rm = TRUE),  # v1 [A+B+C+D+E]
    # prio_i2 = sum(c(b_cs, b_d_sites, b_n_ind, b_n_risk), na.rm = TRUE),         # v2 [B2+C+D+E]
    prio_i3 = sum(c(b_ci, b_d_sites, b_n_ind, b_n_risk), na.rm = TRUE),           # v3 [B+C+D+E]
    prio_i4 = sum(c(b_ci, b_max_a, b_n_risk), na.rm = TRUE),                      # v4 [B+max(C,D)+E]
