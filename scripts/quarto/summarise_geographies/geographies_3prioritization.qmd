...
```{r}
pacman::p_load(tidyverse, here, sf, janitor, skimr, BAMMtools, ggcorrplot, openxlsx)
...
data_prioritized <- geo_enriched %>%
  mutate(across(starts_with("b_"), ~ if_else(.x >= 3, 1, 0), .names = "if_{.col}")) %>%
  ...
  rowwise() %>%
  mutate(
    sum_ind_ifs = sum(c_across(starts_with("if_b_"))),
    sum_risk_ifs = sum(c_across(if_RC1:if_Phy_P3rc))
  ) %>%
  ungroup() %>%
  mutate(
    b_pop_d = case_when(
    ...
    b_n_ind = if_else(sum_ind_ifs > 5, 5, sum_ind_ifs),
    b_n_risk = if_else(sum_risk_ifs > 5, 5, sum_risk_ifs)
  ) %>%
  rowwise() %>%
  mutate(
    b_max_a = max(c(b_d_sites, b_n_ind), na.rm = TRUE), # max of assets-related factors
    b_max_e = max(c(b_ci, b_d_sites, b_n_ind), na.rm = TRUE), # max of economic factors
    prio_i1 = sum(c(b_pop_d, b_ci, b_d_sites, b_n_ind, b_n_risk), na.rm = TRUE),
    # prio_i2 = sum(c(b_cs, b_d_sites, b_n_ind, b_n_risk), na.rm = TRUE),
    prio_i3 = sum(c(b_ci, b_d_sites, b_n_ind, b_n_risk), na.rm = TRUE),
    prio_i4 = sum(c(b_ci, b_max_a, b_n_risk), na.rm = TRUE),
    prio_i5 = sum(c(b_max_e, b_n_risk), na.rm = TRUE)
  ) %>%
  ungroup() %>%
  mutate(across(prio_i1:prio_i5, ~ dense_rank(desc(.x)), .names = "{.col}_rk"))

data_arranged <- data_prioritized %>%
  arrange(desc(d_sites)) %>%
  arrange(desc(sum_risk_ifs))
...
```{r}
geo_enriched <- read_rds(here("output", "summarise_geo", "2_enrichment", "final", "hybas5_enriched.rds")) %>%
```
...
geo_prioritized_regional <- regions_list %>% 
  map(generate_prio_index, top_n_catchments = 50) %>% 
  bind_rows()
```
...
corr <- geo_prioritized_regional %>%
  dplyr::select(`A) Pop Density` = pop_dens,
                ...
                `D) Number of Industries` = sum_ind_ifs, 
                `E) Number of Risks` = sum_risk_ifs, 
...
```{r}
#geo_prioritized_regional <- read_rds(here("output", "summarise_geo", "3_prioritization", "hybas5_prioritized_regional.rds"))
...
geographies$hybas7 %>%
  ...
  dplyr::select(
    ...
    n_ind_ifs = sum_ind_ifs,
    n_risk_ifs = sum_risk_ifs,
    b_pop_d,
    #b_cs,
    b_ci,
    b_d_sites = b_d_sites,
    ...
    b_n_risk,
    prio_i1:prio_i5,
    prio_i1_rk:prio_i5_rk,
    top20_i1:top20_i5,
    # geometry
    geometry
  ) %>%
  st_write(here("output", "summarise_geo", "shp", "hybas5_prioritized_regional.shp"), layer_options = "ENCODING=UTF-8", delete_layer = TRUE)
```

**Get list of WMO basins**

```{r}
basins_i3 <- geo_prioritized_regional %>%
  filter(top_i3 == "Yes") %>%
...
write.xlsx(list(
  ...
  "v4" = basins_i4, 
  "v5" = basins_i5),
  file = here("output", "summarise_geo", "3_prioritization", "hybas5_list_basins_collective_action.xlsx"), rowNames = FALSE)
```
